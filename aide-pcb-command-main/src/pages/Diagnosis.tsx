 ...
 *  3. Display symbolic tree + AI reasoning side-by-side
 */
import {
...
function buildTree(results: VerifyResult[]): TreeNode[] {
  ...
  const overall: "pass" | "warn" | "fail" = fails.length > 0 ? "fail" : warns.length > 0 ? "warn" : "pass";
  ...
  return [
    {
      id: "rail", label: "Voltage Rail Analysis", status: overall,
      children: [
        { id: "pass-rail", label: `Nominal (${passes.length} points)`, status: "pass", children: passes.map(toNode) },
        ...(warns.length > 0 ? [{ id: "warn-rail", label: `Warnings (${warns.length})`, status: "warn" as const, children: warns.map(toNode) }] : []),
        ...(fails.length > 0 ? [{ id: "fail-rail", label: `Failures (${fails.length})`, status: "fail" as const, children: fails.map(toNode) }] : []),
      ],
    },
    { id: "vir", label: "V=IR Verification", status: overall, detail: `${results.length} test points checked` },
  ];
}
  ...
  return (
    <div>
      <button
        ...
        className={`w-full flex items-center gap-2 py-2 px-3 text-xs font-mono rounded transition-colors hover:bg-secondary/50 ${node.status === "fail" ? "text-destructive" : node.status === "warn" ? "text-warning" : "text-foreground"
          }`}
      ...
      {open && hasChildren && node.children!.map((c) => <TreeNodeView key={c.id} node={c} depth={depth + 1} />)}
    </div>
  );
}
...
export default function Diagnosis() {
  ...
  const isOnline = !summaryError;
  const isLoading = summaryLoading || verifying || diagLoading;

  // Build tree whenever verification results change
  useEffect(() => {
    if (results.length > 0) setTree(buildTree(results));
  }, [results]);

  // Verify all test points from the summary
  const runVerification = useCallback(async () => {
    if (!summary) return;
    for (const tp of summary.test_points.slice(0, 6)) {
      await verify(tp.ref, tp.nominal_v, 0.4 + Math.random() * 0.2);
    }
  }, [summary, verify]);

  // Full pipeline: verify → diagnose
  const handleRunDiagnosis = useCallback(async () => {
    let r = results;
    if (r.length === 0 && summary) {
      const fresh: VerifyResult[] = [];
      for (const tp of summary.test_points.slice(0, 6)) {
        const res = await verify(tp.ref, tp.nominal_v, 0.4 + Math.random() * 0.2);
        if (res) fresh.push(res);
      }
      r = fresh;
    }
    setHasRun(true);
    const faults = summary?.active_faults ?? {};
    await runDiagnosis(r, Object.keys(faults).length > 0 ? faults as Record<string, unknown> : null);
    ...
    <div className="h-full flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className={`flex items-center gap-1.5 text-[10px] font-mono px-2 py-1 rounded-full border ${isOnline
              ? "text-success border-success/30 bg-success/10"
              : "text-muted-foreground border-border/30 bg-secondary/50"
            }`}>
            {isOnline ? <Wifi className="w-3 h-3" /> : <WifiOff className="w-3 h-3" />}
            {isOnline ? "BACKEND LIVE" : "DEMO MODE"}
          </div>
          ...
          >
            {verifying ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Play className="w-3.5 h-3.5" />}
            Verify All
          </button>
          ...
          >
            {diagLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Brain className="w-3.5 h-3.5" />}
            Run AI Diagnosis
          </button>
        </div>
      </div>
          ...
          {summaryError && (
            <div className="glass-panel p-3 mb-3 border-warning/20">
              <p className="text-[10px] font-mono text-warning">
                ⚠ Backend offline — start the backend server to enable live analysis.
              </p>
            </div>
          )}
          {tree.length === 0 ? (
        ...
        <div className="w-80 flex flex-col gap-4">
          <div className="glass-panel p-4">
            ...
            <div className="space-y-3">
              {[
                { label: "Total Checks", value: results.length || "—", color: "text-foreground" },
          ...
          <div className="glass-panel p-4 flex-1 overflow-y-auto">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-xs font-mono text-primary uppercase tracking-widest text-glow-cyan">Gemini RCA</h3>
            ...
            {diagResult && (
              <div className="space-y-3">
                {diagResult.thought_process && (
                  <div>
                    <div className="text-[10px] font-mono text-muted-foreground uppercase mb-1">Thought Process</div>
                ...
                <div className="p-3 rounded-md bg-destructive/10 border border-destructive/20">
                  ...
                  <p className="text-[10px] font-mono text-muted-foreground leading-relaxed">{diagResult.rca}</p>
                </div>
                {diagResult.model && (
