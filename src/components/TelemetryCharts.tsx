 ...
 * Connects to the backend WebSocket for real streaming data,
 * with a smooth fallback to synthetic noise when offline.
 */
...
export default function TelemetryCharts({ stressLevel }: Props) {
  ...
  // ── Real telemetry: aggregate all packets → single averaged data point ──────
  useEffect(() => {
  ...
  // ── Synthetic fallback when backend is offline ───────────────────────────────
  useEffect(() => {
    if (isLive) return;
  ...
  return (
    <div className="space-y-3">
      ...
      {chartConfig.map((cfg) => (
        <div key={cfg.key} className="glass-panel p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs font-mono" style={{ color: cfg.color }}>
              {data.length > 0
                ? (data[data.length - 1] as Record<string, number>)[cfg.key].toFixed(cfg.unit === "°C" ? 1 : 2)
                : "—"
              } {cfg.unit}
            </span>
          </div>
